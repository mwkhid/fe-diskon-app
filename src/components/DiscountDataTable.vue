<template>
  <v-data-table
    ...
    show-select
    hover
    class="rounded-0 table-border-bottom"
    density="comfortable"
  >
    <!-- Custom Name Column -->
    <template v-slot:item.name="{ item }">
      <div class="text-body-1 font-weight-medium">
        {{ item.name }}
        <v-chip
          v-if="item._id === newestItemId"
          size="x-small"
          color="light-blue-lighten-4"
          class="text-light-blue-darken-3 ml-2 font-weight-medium"
          label
        >
          Baru
        </v-chip>
    ...
    <template v-slot:headers="{ columns, isSorted, getSortIcon, toggleSort, selectAll, allSelected: isAllSelected, someSelected: isSomeSelected }">
          ...
          <!-- Selection Header -->
            <th v-if="column.key === 'data-table-select'" 
                ...
                <v-checkbox-btn
                    ...
                    color="primary"
                    density="compact"
                    @update:model-value="selectAll(!isAllSelected)"
                ></v-checkbox-btn>
            </th>
          ...
          >
            <div class="d-flex align-center" style="font-size: 16px; font-weight: 800;">
              ...
              <template v-if="column.sortable">
                  ...
                  <v-icon v-else icon="mdi-arrow-up" size="small" class="ml-1 text-grey-lighten-2" style="opacity: 0.5"></v-icon>
              </template>
            </div>
          </th>
        </template>
