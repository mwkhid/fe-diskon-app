            ...
            <v-card class="rounded-xl border" flat>
                ...
                <div class="pa-5 pb-0">
                    <div class="d-flex justify-space-between align-center mb-6">
                        ...
                        <v-btn v-if="!isSelectionMode" color="primary" rounded="pill" prepend-icon="mdi-plus"
                            class="text-capitalize px-6" height="40" @click=" openAddDialog ">
                            Tambah Diskon
                        </v-btn>
                        <div v-else class="d-flex align-center">
                            <v-btn variant="outlined" color="error" rounded="pill" class="text-capitalize px-6 mr-2"
                                height="40" @click=" clearSelection "
                                style="border-color: #EF5350 !important; color: #EF5350 !important">
                                Batalkan
                            </v-btn>
                            <v-btn color="error" rounded="pill" class="text-capitalize px-6" height="40" flat
                    ...
                    <div class="d-flex align-top flex-wrap" style="gap: 12px">
                        ...
                        <div style="width: 240px">
                            <v-text-field v-model=" search " prepend-inner-icon="mdi-magnify" placeholder="Cari diskon"
                        ...
                        <div style="width: auto">
                            <v-menu v-model=" menuOpen " :close-on-content-click=" false " location="bottom start">
                                <template v-slot:activator="{ props }">
                                    <v-btn v-bind=" props " variant="outlined" color="grey-darken-2" rounded="lg"
                                        class="text-capitalize pl-4 pr-3" height="40">
                                        <v-icon icon="mdi-store-outline" start></v-icon>
                                        <span class="mr-2">{{ appStore.apiId ? 'Outlet Active' : 'Pilih Outlet'
                                            }}</span>
                                ...
                                <v-card min-width="400" class="mt-2 rounded-lg elevation-3 pa-4">
                                    ...
                                    <v-text-field v-model=" tempApiId " variant="outlined" density="compact"
                                        placeholder="https://crudcrud.com/api/..." hide-details="auto"
                                        class="mb-4 rounded-lg" bg-color="white"></v-text-field>

                                    <v-btn color="primary" rounded="pill" block height="40" class="text-capitalize"
                                        @click=" applyApiId ">
                                        Terapkan
                                    </v-btn>
                                </v-card>
                ...
                <div class="px-5 pb-5 pt-5">
                    ...
                    <!-- List / Table -->
                    <div v-else>
                        <DiscountDataTable :items=" filteredDiscounts " :loading=" appStore.loading "
                            :newest-item-id=" newestItemId " v-model:selected=" selected " @edit=" openEditDialog " />
                    </div>
                </div>
            </v-card>

            <!-- Dialogs -->
            <DiscountDialog v-model=" showDialog " :edited-item=" selectedItem " @save=" handleSave "
                @delete=" handleDialogDelete " />

            <DeleteDialog v-model=" showDelete " :item-name=" selectedItem ? selectedItem.name : '' "
                :is-bulk=" !selectedItem " @confirm=" handleConfirmDelete " />
        </v-container>
    </template>
