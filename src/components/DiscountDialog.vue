    ...
    <v-dialog v-model=" internalShow " max-width="500px" persistent>
        <v-card class="rounded-xl">
            ...
            <v-card-text class="pt-4 pb-6">
                <v-text-field v-model=" form.name " label="Nama Diskon" variant="outlined" density="comfortable"
                    placeholder="Misal: Diskon opening, diskon akhir tahun" hide-details="auto" class="mb-4 rounded-lg"
                    :error-messages=" errors.name " @input="errors.name = ''"></v-text-field>

                <div class="d-flex align-top">
                    <v-text-field v-model=" form.value " label="Diskon" type="number" variant="outlined"
                        density="comfortable" placeholder="0" hide-details="auto" class="flex-grow-1 mr-3 rounded-lg"
                        :error-messages=" errors.value " @input="errors.value = ''"
                        :prefix=" form.unit === 'Rp' ? 'Rp' : undefined "
                        :suffix=" form.unit === '%' ? '%' : undefined "></v-text-field>

                    <!-- Custom Toggle Group -->
                    <div class="d-flex" style="height: 48px">
                        <div class="d-flex align-center justify-center px-4 cursor-pointer transition-swing"
                            :class=" form.unit === '%' ? 'bg-green-lighten-5 text-green-darken-1 border-primary' : 'bg-white text-grey border-grey-lighten-1' "
                            style="border: 1px solid; border-radius: 30px 0 0 30px; min-width: 80px;"
                            @click="form.unit = '%'">
                        ...
                        <div class="d-flex align-center justify-center px-4 cursor-pointer transition-swing"
                            :class=" form.unit === 'Rp' ? 'bg-green-lighten-5 text-green-darken-1 border-primary' : 'bg-white text-grey border-grey-lighten-1' "
                            style="border: 1px solid; border-left: none; border-radius: 0 30px 30px 0; min-width: 80px;"
                            @click="form.unit = 'Rp'">
            ...
            <v-card-actions class="pa-4">
                <div v-if="isEdit" class="d-flex justify-space-between w-100 align-center">
                    ...
                    <v-btn color="primary" variant="flat" @click=" save " class="text-capitalize px-8" rounded="pill"
                        height="40">
                        Simpan
                    </v-btn>
                </div>
                <v-btn v-else color="primary" variant="flat" @click=" save " class="text-capitalize" rounded="pill"
                    height="40" block>
                    Simpan
                </v-btn>
            </v-card-actions>
